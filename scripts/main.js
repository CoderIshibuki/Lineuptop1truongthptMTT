    ...
    if(event.keyCode == 13) {
        if(n == 0) {
            ...
            if($('#inputPlayer').val() == "summary") {
                showPopup("Thống Kê", "Tổng Số Lượt Truy Cập: <b>" + numberWithCommas(visitCount) + "</b><br>Số Lượng Hình Ảnh Được Lưu: <b>" + numberWithCommas(saveCount) + "</b><br><span style='font-size: 13px; font-style: italic; color: green;'>(2024. 05. 09 Ghi Lại Từ)</span>");
                $('#inputPlayer').val("");
            } else {
                searchPlayers();
            }
...
function searchPlayers(str) {
    if($('#inputPlayer').val() == "summary") {
        showPopup("Thống Kê", "Tổng Số Lượt Truy Cập: <b>" + numberWithCommas(visitCount) + "</b><br>Số Lượng Hình Ảnh Được Lưu: <b>" + numberWithCommas(saveCount) + "</b><br><span style='font-size: 13px; font-style: italic; color: green;'>(2024. 05. 09 부터 기록)</span>");
        $('#inputPlayer').val("");
        return;
    }
    if(allData.length != 0) {
        if($('#inputPlayer').val().length == 0) {
            ...
            if(str != null && str.length != 0) {
                if(str.indexOf("pos:") != -1) {
                    var pos = "";
                    if(str.split("pos:")[1].split("|")[0] == "Tất Cả") {
                        pos = "*";
                    } else {
...
function setTeamColors() {
    ...
    if(teamColor != undefined) {
        if(teamColor.players.length == 1) {
            ...
            teamInfos.players.push(teamColor.players[0].name + ": " + teamColor.players[0].count + "Số Lượng Cầu Thủ");
            ...
            teamInfos.players.push(teamColor.players[0].name + ": " + teamColor.players[0].count + "Số Lượng Cầu Thủ");
            teamInfos.players.push(teamColor.players[1].name + ": " + teamColor.players[1].count + "Số Lượng Cầu Thủ");
        } else {
    ...
    if(nationColor != undefined) {
        ...
        if(cdx != -1) {
            ...
            $('.flag-img > #team-img').attr('title', nationColor.name + ": " + nationColor.count + "Số Lượng Cầu Thủ");
            if(teamColor == undefined) {
                $('#logo1').css({display: 'none'});
                $('#logo2').css({display: 'none'});
            }
...
function teamColorInfo(d) {
    ...
    title.innerHTML = "<b>Thông Tin Màu Đội</b>";
    ...
    avg1.innerHTML = "Độ Tuổi Trung Bình: <b>" + d.avgAge + "Đếm</b>";
    ul.appendChild(avg1);
    var avg2 = document.createElement("li");
    avg2.innerHTML = "Chiều Cao Trung Bình: <b>" + d.avgHeight + "cm</b>";
    ul.appendChild(avg2);
...
function calcAge(birth, bDate) {
	...
	if(bDate != null) {
		if(bDate.replace('-', '').replace('-', '').length != 8) {
			return "Vui Lòng Nhập Chính Xác Ngày";
		} else {
